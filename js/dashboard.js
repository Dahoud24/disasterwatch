const ON_DASHBOARD_PAGE = window.location.pathname.includes('dashboard.html');
...
function getBookmarkStorageKey() {
  ...
  return user ? 'dw_bookmarks_' + user.username : 'dw_bookmarks_guest';
}
...
function renderDashboard() {
  ...
  const html = bookmarks.map(function (bookmark) {
    ...
    const safeDate = escapeHtml(bookmark.date);
  ...
  container.innerHTML = `
    <div class="mb-3">
      <p class="text-muted">
        Logged in as <strong>${escapeHtml(user ? user.username : 'Guest')}</strong>.
